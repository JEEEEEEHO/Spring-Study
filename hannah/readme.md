...
- 스레드(thread) : 실행 흐름의 단위(사전적)
    - 하드웨어적 스레드
        ...
        - 1코어 1스레드 CPU(상) / 2코어 4스레드(하)
          => 이처럼 하나의 코어로 여러가지 명령어를 동시 처리하는 CPU를 멀티스레드(multithread) 또는 멀티스레드 CPU라고 한다.
    ...
    - 소프트웨어적 스레드
      - 하나의 프로그램에서 독립적으로 실행되는 단위
        ![image](https://github.com/user-attachments/assets/8aa7556b-8afc-4f31-93e7-4c2f8b47ef7c)
        ![image](https://github.com/user-attachments/assets/9dba7c29-9edd-47ae-9954-351b2a6a5b57)

        
#### 4) 멀티 스레드 프로세서
  - 하나의 코어로 여러 개의 명령어를 동시에 실행할 수 있는 CPU
  - 핵심은 **레지스터,** 여기서는 임의로 레지스터 세트로 표현함
    ![image](https://github.com/user-attachments/assets/2f6c2c02-7c12-4b56-b6eb-742d69475e71)
  - 2코어 4스레드는 한번에 네 개의 명령어를 처리 할 수 있음
   ⇒ 프로그램 입장에서는 CPU가 네 개 있는 것처럼 보임
  ⇒ 논리 프로세서 logical processor 이라고 부르기도 함
